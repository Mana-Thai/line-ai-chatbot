...
- **価格を設定するには**: 管理者モードにした後、単価表セクションの「価格を設定」から
  工賃込みの一律価格(サイズ別+持ち込み)を入力。全員の画面に単価と金額が即時反映されます
- **デザインのサンプル画像**: 同じ「価格を設定」画面から、各デザイン(胸ロゴ5種+バックプリント有)の
  サンプル画像をアップロード可能。注文フォームの選択肢の横に小さく表示されます
- **発注するとき**: 集計セクションの「CSV 日本語」または「CSV ไทย(タイ語)」ボタンで
  明細CSV(単価・金額入り)をダウンロード。Excelでそのまま開けます
- **支払いを受けたとき**: 支払った本人が自分の注文カードの「支払い済み」にチェック。
  名前別の合計に支払い状況と未収額が自動表示されます(チェックできるのは入力した本人と管理者のみ)
...
- **AI制作ツールを使うとき**(画像生成 `artwork/tools/stylize.py` / ドラマ動画生成
  `gift-video/scripts/drama_clip.py`): `GEMINI_API_KEY` が必要ですが、**これは手元のPCで
  動かすツール専用**です。Renderの環境変数には追加しません(注文アプリ本体では使いません)。
  取得先は https://aistudio.google.com/apikey で、リポジトリ直下の `.env` に書きます。
  ※動画生成(Veo)は無料枠では使えず、キーに有料課金の設定が必要です
