...
4. 「Environment」で環境変数を設定:
   ...
   | `APP_PASSCODE` | グループの合言葉(自由に決める。例: `tshirt2026`) |
...
3. メンバーがリンクをタップすると、LINE内でアプリが開きます。
   初回だけ合言葉を入力すれば、以降はすぐ注文画面が表示されます。
...
- **発注するとき**: 集計セクションの「CSV出力」ボタンで明細CSVをダウンロード。
  Excelでそのまま開けます(画面の集計表はデザイン×サイズ×カラー別の枚数)
- **合言葉を変えたいとき**: Renderの環境変数 `APP_PASSCODE` を変更して再デプロイ。
  メンバーは次回アクセス時に新しい合言葉を再入力します
