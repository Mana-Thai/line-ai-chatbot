...
取得済みドメインの設定手順と、取得時の記録。**残っている作業は「DNS設定」以降。**

## 取得済み: `aniostea.com`

**2026-08-01、Cloudflare Registrar で取得完了。**

| 項目 | 内容 |
|---|---|
| ステータス | Active |
| 有効期限 | 2027-08-01(自動更新ON・$10.46/年) |
| ネームサーバー | `desiree.ns.cloudflare.com` / `lynn.ns.cloudflare.com` |

以下は取得時の記録と、**購入後に必要な設定**(DNS・SSL・サイト書き換え)。
...
## 取得手順(Cloudflare Registrar)— 実施済み・次のドメイン取得時の参考

1. Cloudflare のアカウントを作る(無料プランでよい)
2. ダッシュボード → **Domain Registration → Register Domain** でドメインを検索
3. **購入画面で空き状況を必ず再確認する**(調査から時間が経つと他者に取られている場合がある)
