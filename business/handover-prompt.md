...
- アート感情設計の正本: `business/art-emotion-playbook.md`
  (10か条・納品前チェックリスト・季節代表作カレンダー。全作品・全告知に適用)
...
- ブランチはoriginと同期済み。
- 計画はv4(アート感情戦略入り)。感情設計の正本は `business/art-emotion-playbook.md`。
  全作品の制作・納品はプレイブックの10か条と§5チェックリストに従う。
- 現行代表作は母の日マッチカット短編v7。月商20万THBの商品階段・90日営業KPI、
  ポートフォリオv7、母の日告知v4へ更新済み。
...
## 15. 計画v4 アート感情戦略（2026-07-26追加）

- オーナー指示「月々20万THBへ、アートの要素を増やして感情を動かす」を受け、
  計画をv4へブラッシュアップ。
- `business/art-emotion-playbook.md` を新設。v3〜v7の試行錯誤で得た感情設計を
  再現可能な型として文書化: 10か条（説明しない/顔より手/マッチカット/沈黙/涙は隠す/
  怖さ連想チェック/低彩度+記憶色1つ/文字は一行/具体が普遍を語る/固有要素1つ）、
  商品階段とアート深度の対応（価格差は長さでなく深さで説明）、季節代表作エンジン
  （母の日→10月敬老→父の日12/5→年末年始→2/14→ソンクラーン4/13、各3週間前に制作開始）、
  納品前の感情チェックリスト。
- `business/income-plan.md` に §2.5「アート戦略」を追加し、クロスセル導線・
  週次ルーティン・短期プランの現況（7/26: 残るボトルネックは本人の個別送信のみ、
  受注3件まで制作改善は凍結）を更新。
- 新しい表現の実験は代表作に限定し、受注制作は実証済みの型+固有要素で行う方針を明文化。
- 公開URL:
