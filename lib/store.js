...
class PgStore {
    ...
    async init() {
        ...
        `);
        // 旧バージョンで作成済みのテーブルへの追加(注文者名の自由入力対応)
    ...
    async create(o) {
        const { rows } = await this.pool.query(
            `INSERT INTO orders (user_id, display_name, order_name, chest_logo, back_print, size, quantities, note, updated_by)
             VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9) RETURNING *`,
            [o.userId, o.displayName, o.orderName, o.chestLogo, o.backPrint, o.size,
    ...
    async update(id, f) {
        const { rows } = await this.pool.query(
            `UPDATE orders SET order_name = $1, chest_logo = $2, back_print = $3, size = $4,
                quantities = $5, note = $6, updated_by = $7, updated_at = now()
             WHERE id = $8 RETURNING *`,
            [f.orderName, f.chestLogo, f.backPrint, f.size, JSON.stringify(f.quantities), f.note, f.updatedBy, id],
        );
        return rows[0] ? this._row(rows[0]) : null;
    }
