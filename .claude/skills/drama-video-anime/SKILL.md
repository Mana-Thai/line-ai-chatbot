...
セリフの有無で決める。`--provider veo`(既定)はセリフの口パク音声を同時生成できる代わりに
高く、`--provider kling`(要 `FAL_KEY` / https://fal.ai)は安く人物の一貫性が強い。
**セリフの無い作品なら kling** が費用対効果で明確に上(詳細は `drama-video-liveaction` Skill)。
kling はクリップが5秒/10秒で、混ぜて緩急をつけられる。
