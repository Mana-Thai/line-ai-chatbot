...
(2人写るシーンはリストで複数指定できる)。BGMが無ければ確認用のプレースホルダーを
自動生成する(本番では差し替える)。仮の絵コンテには印が付くので、あとで本生成を
実行すれば**自動で実素材に置き換わる**(消してから生成し直す必要はない)。
ここで「シーンが多い/少ない」「この流れで良いか」を潰してから生成に進むと、
リテイクの課金を大きく減らせる。

### 1.6. 無料枠のWeb UIで質感を試す(任意)

Google Flow は非契約者にも1日分の無料クレジットがあるので、課金前に**実際の生成映像**の
質感を確認できる。API経由では無料枠が無いため、これはWeb UIを手で操作する作業になる:

```bash
# 脚本から「貼り付けるだけ」のプロンプト集を書き出す(APIを呼ばないので無料)
python3 scripts/drama_clip.py --scenes orders/x-001/drama.yaml \
    --out-dir orders/x-001/input --export-prompts orders/x-001/prompts.md
```

依頼者(または自分)が Flow でプロンプトを貼り、参照画像をアップして生成 → mp4を
`orders/x-001/input/sceneN.mp4` として保存すれば、そのまま assemble に乗る。

**注意: 生成サービスを混ぜると見た目が揃わない。** Flowで一部・Klingで残りを作ると、
色味・粒状感・動きの質が変わって1本の作品として不自然になる。無料枠を使うなら
**全シーンをFlowで作る**か、**Flowは質感の下見だけにして本番は1つのサービスで通す**か、
どちらかにする。
