...
1. **Gemini API(推奨・自動化できる)**: 環境変数 `GEMINI_API_KEY` が必要。
   Veo(動画生成)に**無料枠は無く**、キーを作った上でそのプロジェクトの課金を
   有効化しないと動かない。料金は**出力1秒あたり** Lite $0.05 / Fast $0.10 /
   標準 $0.40(720p基準)で、8秒生成すると1本 $0.4〜$3.2。**実行前に必ず本数と
   概算費用を伝えて確認を取る**。使えるモデルは `--list-models` で確認できる
   (Veo 3.0/2.0系は2026-06-30に停止済み。既定は `veo-3.1-fast-generate-001`)
...
同じ登場人物が複数シーンに出る動画では、シーンごとに生成すると**顔が別人になる**。
参照画像を渡して防ぐ:

```bash
python scripts/i2v.py scene3.png --prompt "..." --out scene3.mp4 \
    --ref refs/mother.png --ref refs/daughter.png
```

- `--ref` は**1シーンあたり最大3枚**。人物ごとに1枚、顔がはっきり大きく写った
  正面〜斜め45度の画像を選ぶ(全身の引き画は効きにくい)
- 画風を揃えたいときは `--style-ref`(1枚)。**`--ref` とは併用できない**(Veoの制約)
- `--ref` だけ渡して `src` を省略すると、開始フレーム無しで参照画像だけから生成できる
- 作中で年齢が変わる人物は、若年期・高年期で別の参照画像を用意して使い分ける
- モデルによっては参照画像に未対応で400が返る。その場合は `--model` を変えるか、
  プロンプトに人物の特徴(年齢・髪型・服装・体型)を書き込む方法に切り替える
